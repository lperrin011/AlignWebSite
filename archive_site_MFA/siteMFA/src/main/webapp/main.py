...
if len(sys.argv) == 1:
    ...
    DIRECTORY = sys.argv[1] #the directory where to search the file is in parameter
    # FILE_NAME_WAV = list(sys.argv)[1]
    wav_files = glob.glob(os.path.join(DIRECTORY, "*.wav")) #search the wav file in the good directory
    if not wav_files:
        raise ValueError(f"No .wav file found in the directory '{DIRECTORY}'")
    FILE_NAME_WAV = wav_files[0]
...
wav_path = os.path.join(ORIGINAL_DIRECTORY, FILE_NAME_WAV) #os.getcwd() + "/" + FILE_NAME_WAV
...
with open("main_log.txt", "w") as f:
    ...
    bash_out = s.run(f"cp {FILE_NAME_WAV} data/test/{os.path.basename(FILE_NAME_WAV)}", stdout=f, text=True, shell=True)
    ...
    if(len(sys.argv) ==5): #in case url addresses are provided
        for i in [2,3,4]:
            tarball = sys.argv[i].split("/")[-1] #take the name of the model
            if tarball not in os.listdir():
                bash_out = s.run('wget {0}'.format(sys.argv[i]), stdout=f, text=True, shell=True)
    ...
    if len(sys.argv) == 1:
        ...
        output_file_name = os.path.basename(FILE_NAME) + ".txt" #the name of the input audio
        output_file_path = os.path.join(DIRECTORY, output_file_name)
        command = "../../../src/latbin/lattice-best-path " \
          "ark:'gunzip -c {0} |' " \
          "'ark,t:| utils/int2sym.pl -f 2- " \
          "{1} > {2}'".format(gz_location, words_txt_loc, output_file_path)
